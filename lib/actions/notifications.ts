...
export async function sendFeedbackAlertEmail(
  ...
  try {
    ...
    const escapedName = escapeHtml(learnerName);
    const escapedEmail = escapeHtml(learnerEmail);
    const escapedClarity = escapeHtml(clarity.replace(/-/g, ' '));
    const escapedDifficulty = escapeHtml(difficulty.replace(/-/g, ' '));
    const escapedChallenge = escapeHtml(challenge);
    const escapedSuggestions = escapeHtml(suggestions);
    const escapedRecommend = escapeHtml(wouldRecommend);

    const result = await resend.emails.send({
      from: "Learn to Vibe Code <noreply@learntovibe.code>",
      to: "support@learntovibe.code",
      subject: `New Learner Feedback from ${escapedName}`,
      html: `
        <h2>New Feedback Received</h2>
        <p><strong>From:</strong> ${escapedName} (${escapedEmail})</p>
        <hr/>
        <p><strong>📚 Content Clarity:</strong> ${escapedClarity}</p>
        <p><strong>⚙️ Difficulty Level:</strong> ${escapedDifficulty}</p>
        <p><strong>🚧 Biggest Challenge:</strong></p>
        <p style="background: #f5f5f5; padding: 12px; border-left: 3px solid #7C3AED;">${escapedChallenge.replace(/\n/g, '<br>')}</p>
        <p><strong>✨ Suggestions:</strong></p>
        <p style="background: #f5f5f5; padding: 12px; border-left: 3px solid #7C3AED;">${escapedSuggestions.replace(/\n/g, '<br>')}</p>
        <p><strong>🌟 Would Recommend:</strong> ${escapedRecommend}</p>
        <hr/>
        <p style="font-size: 12px; color: #666;">This is automated feedback from ${escapedName}. Review in Supabase dashboard.</p>
      `,
    });
