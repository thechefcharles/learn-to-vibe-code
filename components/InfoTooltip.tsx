    ...
    <div className="relative inline-block w-full">
      ...
      <AnimatePresence>
        {isHovering && (
          ...
          >
            <div className="bg-ink/95 text-paper rounded-lg px-4 py-3 shadow-xl max-w-xs backdrop-blur-sm">
              <p className="font-bold text-sm mb-1">{title}</p>
              <p className="text-xs leading-relaxed opacity-90">{description}</p>
              {/* Tooltip arrow */}
              <div className="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-ink/95 rotate-45"></div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
